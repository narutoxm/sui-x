...
impl ObjectCacheRead for InputLoaderCache<'_> {
    ...
    fn multi_input_objects_available_cache_only(&self, keys: &[InputKey]) -> Vec<bool> {
        let mut available = self.loader.cache.multi_input_objects_available_cache_only(keys);
        for (idx, key) in keys.iter().enumerate() {
            let in_override_cache = match key {
                InputKey::VersionedObject { id, version } => self
                    .cache
                    .iter()
                    .any(|(cache_id, obj)| cache_id == &id.id() && obj.version() == *version),
                InputKey::Package { id } => self
                    .cache
                    .iter()
                    .any(|(cache_id, obj)| cache_id == id && obj.is_package()),
            };
            if in_override_cache {
                available[idx] = true;
            }
        }
        available
    ...
    fn notify_read_input_objects<'a>(
        ...
        Box::pin(async move {
            let filtered_keys = input_and_receiving_keys
                .iter()
            ...
            self.loader
                .cache
                .notify_read_input_objects(&filtered_keys, receiving_keys, epoch)
                .await;
        })
    }
}
